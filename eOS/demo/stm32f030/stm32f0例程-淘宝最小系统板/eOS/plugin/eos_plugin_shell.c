...
os_u32 os_shell_uint_get(os_u8 n);//提取字符串中相应位置的无符号整型数字
os_u8 os_shell_hex_get(os_u8 n);//提取字符串中相应位置的无符号整型数字
os_u32 os_shell_hex32_get(os_u8 n);//提取字符串中相应位置的无符号整型数字
//
...
例1：	if(os_shell_compare_dig("led time=%d",'=')) { led_timer_set( os_shell_float_get(0) ); }
      ...
      //分别为2020、2、5，被非数字字符隔开的数字视为不同数字，提取到的数字统一为os_u32类型。如果指令需要带浮点数，
...
//类型  ：字符串处理函数
/*********************************************/
os_u8 os_shell_compares(os_u32 offset,char* p)
{
...
//形参  ：os_u8 n位置
//返回值：os_u32型的一个无符号整型数
//作用  ：获取字符串p中的第n个数字(n=0开始)
//类型  ：字符串处理函数
/*************************************************/
os_u32 os_shell_uint_get(os_u8 n)
{
...
//类型  ：字符串处理函数
/*************************************************/
os_u8 os_shell_hex_get(os_u8 n)
{
...
//类型  ：字符串处理函数
/*************************************************/
os_u32 os_shell_hex32_get(os_u8 n)
{
...
void string_float_convert(char* ptr_source,double *outbuff,os_u32 len)
{
	...
	while(*p!='\0') 
	{
		if((*p>='0')&&(*p<='9'))//数字
		{
			if(*(p-1)=='-') neg_flag=OS_TRUE;//判断数字前面的非数字符号是否是负号
			while((*p>='0')&&(*p<='9'))
			{
			...
			if(neg_flag==OS_TRUE) outbuff[i]=0-outbuff[i];
			i++;
			if(i>=len) break;
		} 
		p++;
