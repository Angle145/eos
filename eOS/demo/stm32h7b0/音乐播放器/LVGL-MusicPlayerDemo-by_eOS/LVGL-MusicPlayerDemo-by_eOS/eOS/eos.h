...
/********************************************OS参数配置*************************************************************/
#define OS_APP_USER_MAX       8			//用户APP数量(0-62,内存使用为N*(10~12)) bytes)
#define OS_STACK_SIZE         1024*10	//用户APP栈内存,单位：word
#define OS_APP_SLICETIME      1			//APP时间切片,单位：tick(默认1ms,在eos_port2.c/systick_init()修改)(范围：1-200)  
#define STK_IDLE_SIZE         128		//空闲APP栈长度 (内存使用为N*4) bytes) >=32
#define STK_CORE_SIZE 		  256		//系统核心栈长度	 (内存使用为N*4) bytes) >=32	（和用户APP数量+软定时器数量有关）	
...
#define OS_TIMER_MAX	      1			//系统软定时器数量(ID数量)
...
#define OS_MAIL_SIZE		  128		//系统邮件长度
#define OS_QUEUE_SIZE		  16		//邮箱队列深度(1个queue_id能缓存的邮件数)
	
#define _RAM_ALGO_            1			//0-rom,1-ram,算法数组保存位置。
...
//APP状态
#define APP_STATE_RUNNING    0  //运行态
#define APP_STATE_PENDING    1  //挂起态
#define APP_STATE_RESTART    2	//重启态
#define APP_STATE_DELETED    3	//删除态
...
//#define	os_dwg_feed_api()	fwdgt_feed()
